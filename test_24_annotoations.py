...
photo_dir = "dataset/24_chromosomes_object/JEPG"  
xml_dir = "dataset/24_chromosomes_object/annotations"  
...
def image_processing():
    show_image = image_files[4555] #cia keisti kuria foto
    ...
    if os.path.exists(xml_file):  # jei egzisutuoja 
        ...
        # image = drawing_annotation(image, xml_file) 
        image= detect_objects_yolov8(image)# nupiesia anotacija

        cv2.imshow("annotated chromosomes", image)
        cv2.waitKey(0)  
